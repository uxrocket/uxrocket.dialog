...
(function($) {
    ...
    var bindUIActions = function($el) {
        ...
        $body.on(events.click, ".uxitd-dialog-instance-" + _opts.instance + " .uxitd-dialog-confirm-button", function(e) {
            e.preventDefault();
            callback(_opts.onConfirm);
            ux.close(_opts);
        });

        $body.on(events.click, ".uxitd-dialog-instance-" + _opts.instance + " .uxitd-dialog-cancel-button", function(e) {
            e.preventDefault();
            callback(_opts.onCancel);
            ux.close(_opts);
        });

        $body.on(events.click, ".uxitd-dialog-instance-" + _opts.instance + " .uxitd-dialog-close", function(e) {
            e.preventDefault();
            ux.close(_opts);
        });
    };
    ...
    var buildDialog = function(opts) {
        var html = '' +
                   '<div id="uxitd-dialog" class="uxitd-dialog-instance-' + opts.instance + ' ' + opts.className + '">';

        if(opts.blockUI === true) {
            html += '\n' +
