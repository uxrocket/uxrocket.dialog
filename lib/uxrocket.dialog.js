...
(function($) {
    ...
    Dialog.prototype.bindUIActions = function() {
        ...
        $body.on(events.click, '#uxitd-dialog-' + _this._instance + ' .uxitd-dialog-confirm-button', function(e) {
            e.preventDefault();
            utils.callback(_this.options.onConfirm);
            ux.close(_this);
        });

        $body.on(events.click, '#uxitd-dialog-' + _this._instance + ' .uxitd-dialog-cancel-button', function(e) {
            e.preventDefault();
            utils.callback(_this.options.onCancel);
            ux.close(_this);
        });

        $body.on(events.click, '#uxitd-dialog-' + _this._instance + ' .uxitd-dialog-close', function(e) {
            e.preventDefault();
            ux.close(_this);
        });
    };
    ...
    Dialog.prototype.prepare = function() {
        var html = '' +
                   '<div id="uxitd-dialog-' + this._instance + '" class="uxitd-dialog uxitd-dialog-' + this.options.type + ' ' + this.options.className + '">';
        ...
        html += '\n' +
                '<div id="uxitd-dialog-content" class="note dialog-note note-' + this.options.type + '">';

        if(this.options.title !== '') {
            html += '<h3 class="uxitd-dialog-title note-title">' + this.options.title + '</h3>';
        }

        if(this.options.message !== '') {
            html += '<div class="uxitd-dialog-content note-content">' + this.options.message + '</div>';
        }
        ...
        if(this.options.confirm.show || this.options.cancel.show || this.options.buttons.length > 0) {
            ...
            if(this.options.cancel.show) {
                html += '<a href="#" class="uxitd-dialog-button uxitd-dialog-cancel-button ' + this.options.cancel.className + '">' + this.options.cancel.text + '</a>';
            }
            ...
            if(this.options.confirm.show) {
                html += '<a href="#" class="uxitd-dialog-button uxitd-dialog-confirm-button ' + this.options.confirm.className + '">' + this.options.confirm.text + '</a>';
            }
